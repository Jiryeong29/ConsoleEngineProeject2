...
AStar::AStar(GameLevel* level)
	: gameLevel(level)
{
	Node* start = new Node(Vector2(level->GetPlayerPosition().x, level->GetPlayerPosition().y));
	Node* end = new Node(Vector2(level->GetEndPosition().x, level->GetEndPosition().y));
...
std::vector<Node*> AStar::FindPath(const std::vector<std::vector<char>>& maze)
{
	Node* start = new Node(Vector2(gameLevel->GetPlayerPosition().x, gameLevel->GetPlayerPosition().y));
	Node* end = new Node(Vector2(gameLevel->GetEndPosition().x, gameLevel->GetEndPosition().y));
...
bool AStar::IsInRange(int x, int y, const std::vector<std::vector<char>>& maze)
{
...
//
//void AStar::DisplayGridWithPath(std::vector<std::vector<char>>& maze, const std::vector<Node*>& path)
//{
//	for (const Node* node : path)
//	{
//		// 경로는 '2'로 표시.
//		maze[node->position.y][node->position.x] = 2;
//	}
//
